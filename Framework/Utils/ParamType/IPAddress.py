...
class IPAddress(ParamTypeBase):

    def __init__(self, typeAndValidation):
        super(IPAddress, self).__init__(typeAndValidation)

    @staticmethod
    def __isIPV6Address(ip):
        ...
        return re.match(ipv6Reg, ip)

    @staticmethod
    def __isIPV4Address(ip):

        ipv4Reg = '(((2[0-5][0-5])|([01]?[0-9]{1,2}))\.){3}((2[0-5][0-5])|([01]?[0-9]{1,2}))$'
        return  re.match(ipv4Reg, ip)

    def getValueInput(self, ip):

        if ip is None or ip == 'None':
            return None
        if IPAddress.__isIPV4Address(ip) or IPAddress.__isIPV6Address(ip):
            return ip
        raise ATException('IPAddress object getValueInput() failed. \nip(%s) is not a IP address.' % str(ip))